...
export default function NewTeamMember() {
  ...
  const [form, setForm] = useState({
    name: '', role: '', bio: '', imageUrl: '', linkedin: '', email: '', order: 0, isActive: true,
  });
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <div>
          <h1 style={{ color: '#1E4C81', fontSize: '26px', fontWeight: 800, margin: 0 }}>👤 Nouveau Membre</h1>
          <p style={{ color: '#888', fontSize: '14px', marginTop: '4px' }}>Ajoutez un membre à l'équipe ATR</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '30px', maxWidth: '900px' }}>
            ...
            <div style={{ display: 'grid', gap: '20px' }}>
              ...
              <div>
                ...
                <textarea value={form.bio} onChange={e => set('bio', e.target.value)} rows={4}
                  style={{ ...inputStyle, resize: 'vertical' }} placeholder="Courte biographie du membre..."
                  onFocus={e => e.target.style.borderColor = '#1E4C81'} onBlur={e => e.target.style.borderColor = '#e8edf5'} />
              </div>
              <div>
                ...
                <input value={form.imageUrl} onChange={e => set('imageUrl', e.target.value)} style={inputStyle}
                  placeholder="https://... ou /images/team/photo.jpg"
                  onFocus={e => e.target.style.borderColor = '#1E4C81'} onBlur={e => e.target.style.borderColor = '#e8edf5'} />
              </div>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', position: 'sticky', top: '20px' }}>
            <div style={{ background: 'linear-gradient(135deg, #1B5299 0%, #2563eb 100%)', padding: '30px 20px', textAlign: 'center' }}>
